...
interface Distribution {
  ...
  receiving_organization_unit: { name: string; district: string }[];
...
const CondomDistrictDistribution: React.FC<ChartProps> = ({ data }) => {
      ...
      data.forEach((item) => {
        if (item.distributions) {
          item.distributions.forEach((distribution) => {
            if (distribution.receiving_organization_unit.length > 0) {
              const org = distribution.receiving_organization_unit[0];
              const district = org.district; // Get the district name
              if (receivingOrgCounts[district]) {
                receivingOrgCounts[district] += parseInt(distribution.transaction_quantity, 10);
              } else {
                receivingOrgCounts[district] = parseInt(distribution.transaction_quantity, 10);
              }
            }
    ...
    <div className="col-span-12 w-full rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5">
      <div>
        <h3 className="text-xl font-semibold text-black dark:text-white">
          Total Condoms Distributed To Each District
        </h3>
      </div>
...
export default CondomDistrictDistribution;
