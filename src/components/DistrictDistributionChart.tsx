...
const DistrictDistributionChart: React.FC<ChartProps> = ({ data }) => {
  ...
  useEffect(() => {
      ...
      if (item.distributions) {
        item.distributions.forEach((distribution) => {
          distribution.receiving_organization_unit.forEach((org) => {
            if (receivingOrgCounts[org.district]) {
              receivingOrgCounts[org.district]++;
            } else {
              receivingOrgCounts[org.district] = 1;
            }
          });
        });
      }
    ...
    setSeries(Object.values(receivingOrgCounts));
    setLabels(Object.keys(receivingOrgCounts));
  }, [data]);
    ...
    <div className="col-span-12 w-full rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5">
      <div>
        <h3 className="text-xl font-semibold text-black dark:text-white">
          Total Number Of Times Ditributed
        </h3>
      </div>
