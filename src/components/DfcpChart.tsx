...
const DfcpChart: React.FC<ChartFourProps> = ({ data }) => {
  ...
  useEffect(() => {
    ...
    data.forEach((item) => {
      item.distributions.forEach((distribution) => {
        quantities.push(parseInt(distribution.transaction_quantity, 10));
        categories.push(`Item ${item.id} Dist ${distribution.id}`);
      });
    });
